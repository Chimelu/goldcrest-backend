...
import { executeMarketBuy, executeMarketSell } from '../services/trade/marketOrder';
...
router.get('/summary', async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const holdings = await CryptoHolding.findAll({
      where: { userId },
      order: [['symbol', 'ASC']],
    });
...
router.post('/buy', async (req: AuthRequest, res) => {
  ...
  try {
    const result = await executeMarketBuy({ userId, symbol, spendUsd });
    return res.json({ message: 'Buy executed', ...result });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Buy failed';
    const code =
      msg.includes('Insufficient') || msg.includes('not available')
        ? 400
...
router.post('/sell', async (req: AuthRequest, res) => {
  ...
  try {
    const result = await executeMarketSell({ userId, symbol, cryptoAmount });
