...
function addUsdString(current, add) {
...
router.post('/users/:userId/top-up', async (req, res) => {
    ...
    try {
        const result = await models_1.sequelize.transaction(async (t) => {
            const user = await models_1.User.findByPk(userId, { transaction: t, lock: t.LOCK.UPDATE });
            if (!user) {
...
router.patch('/withdrawals/:id', async (req, res) => {
    ...
    try {
        const result = await models_1.sequelize.transaction(async (t) => {
            const wr = await models_1.WithdrawalRequest.findByPk(id, {
                transaction: t,
                lock: t.LOCK.UPDATE,
            });
            if (!wr) {
            ...
            if (wr.status !== 'PENDING') {
                return { ok: false, message: 'Only PENDING withdrawals can be updated' };
            }
            const ledger = await models_1.AccountTransaction.findOne({
